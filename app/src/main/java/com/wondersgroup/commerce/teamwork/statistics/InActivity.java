...
//        params.put("organId", "510000000");
    ...
    private void initData() {
        ...
        ApiManager.tjApi.getExceptStatInInfo(params).enqueue(new Callback<In>() {
            @Override
            public void onResponse(Response<In> response, Retrofit retrofit) {
                MyProgressDialog.dismiss();
                In result = response.body();
                if (result != null && result.getResult() != null) {
                    if (result.getCode() == 200) {
                        if (adapter == null) {
                            adapter = new InOutAdapter(InActivity.this, generateData(result.getResult()));
                            adapter.setOnItemClickListener(new InOutAdapter.OnItemClickListener() {
                                ...
                                public void onClick(View v, int position) {
                                    ...
                                    intent.setClass(InActivity.this, ReasonActivity.class);
                                    intent.putExtras(bundle);
                        ...
                        if (adapter == null) {
                            adapter = new InOutAdapter(InActivity.this, generateEmptyData());
                            recyclerView.setAdapter(adapter);
                        } else {
    ...
    private List<In.InOutItem> generateData(In.InResult result) {
        List<In.InOutItem> data = new ArrayList<>();
        In.InOutItem item = new In.InOutItem();
        item.setName("企业");
        item.setType("QY");
        item.setdCount(result.getQY_IN_D_COUNT());
        item.setCount(result.getQY_IN_COUNT());
        item.setTotalDCount(result.getTOTAL_QY_IN_D_COUNT());
        item.setTotalCount(result.getTOTAL_QY_IN_COUNT());
        data.add(item);
        item = new In.InOutItem();
        item.setName("农民专业合作社");
        item.setType("NM");
        item.setdCount(result.getNM_IN_D_COUNT());
        item.setCount(result.getNM_IN_COUNT());
        item.setTotalDCount(result.getTOTAL_NM_IN_D_COUNT());
        item.setTotalCount(result.getTOTAL_NM_IN_COUNT());
        data.add(item);
        item = new In.InOutItem();
        item.setName("个体");
        item.setType("GT");
        item.setdCount(result.getGT_IN_D_COUNT());
        item.setCount(result.getGT_IN_COUNT());
        item.setTotalDCount(result.getTOTAL_GT_IN_D_COUNT());
        item.setTotalCount(result.getTOTAL_GT_IN_COUNT());
        data.add(item);
        return data;
    }
