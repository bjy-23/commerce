...
public class ExampleInfoAdapter extends AnimatedExpandableListView.AnimatedExpandableListAdapter {
	...
	public ExampleInfoAdapter(LayoutInflater inflater, List<Father> fathers) {
		super();
	...
	public Child getChild(int groupPosition, int childPosition) {
		Child Child = fathers.get(groupPosition).getChildMatter()
				.get(childPosition);
		return Child;
	}
...
//					.findViewById(R.id.radioButton2);
		...
		if (child.getChildFlag() == 1) {
			childHolder.subTitle.setText(child.getMatterName()+"    是");
		} else if (child.getChildFlag() == 0) {
			childHolder.subTitle.setText(child.getMatterName()+"    否");
		} else {
